...
**Estado:** EN PROGRESO (decisión de iniciar confirmada por el dueño del proyecto, 2026-09-16) — todavía sin código
**Motivo:** Se prevé integrar otros tipos de perfil a futuro. Admin, Líder de área y Coordinador de cuenta deben poder crear/editar/eliminar roles y asignarles permisos **granulares** (por acción, no todo-o-nada) desde una pantalla de administración, en vez de que estén fijos en código (`lib/authz.ts`) como hoy.
**Alcance confirmado (2026-09-16):** CRUD completo de roles desde `/admin`, con asignación de permisos granular por acción/recurso — la misma dimensión "acción × alcance (todo/mis clientes/asignadas)" que ya está documentada como matriz de permisos en el artifact del roadmap. Incluye migrar `User` de un `role` único a multi-rol (`user_roles`, ver ADR-005) — es la misma migración de esquema, no dos trabajos separados.
**Por qué no se hizo en la misma sesión que Nuevo #13/#14/#16/#20:** toca ~20 archivos que hoy chequean `user.role === "..."` directamente (`app/actions.ts`, `lib/authz.ts`, páginas de admin, `lib/session.ts`, rutas de API, etc.) — es la pieza de mayor esfuerzo y más sensible en seguridad de todo el roadmap. Se diseña el esquema (`Role`, `UserRole`, catálogo de permisos) y se valida antes de tocar código de autorización en producción.
**Mientras tanto:** `Rec. #21` (Diseño/SEO/Desarrollo ven solo lo asignado) y `Rec. #22` (Coordinador ve solo sus clientes) siguen resueltos hoy de forma hardcodeada en `lib/authz.ts` (`requestVisibilityWhere`, `clientVisibilityWhere`, `canActOnRequest`) — se reemplazan sin drama cuando este ADR tenga código.
**Extensión (2026-09-01):** el mismo criterio de "hardcodeado por ahora, editable cuando llegue `ADR-011`" se aplica también a las pantallas de administración de `Rec. #27-#30` (clientes, usuarios, equipos) — `/admin` queda restringido a `ADMIN` únicamente, no a todo `isManager()`, porque crear logins, desactivar personas y editar horas contratadas es más sensible que solo ver datos. Líder de área y Coordinador de cuenta ganan estas capacidades cuando este ADR tenga código.

### ADR-005 — El rol `CLIENTE` del modelo `User` de Prisma se retira en favor de un modelo multi-rol tipo `user_roles`, portado a RGC
**Estado:** SUPERADA en parte (2026-09-16) — el retiro de `CLIENTE` se **rechaza**; el multi-rol para el staff interno **se aprueba** y pasa a ADR-011
**Motivo original:** Codia Task modela "un usuario es de un cliente y tiene uno o más roles" de forma más completa (`users.client_id` + `user_roles` multi-rol, confirmado en la auditoría técnica del 2026-09-01).
**Qué cambió (2026-09-16):** el dueño del proyecto confirmó que `User.role = "CLIENTE"` junto con `clientId` se **mantiene tal cual** — da exactamente la separación equipo interno/portal que se necesita, y cambiar la representación interna no le agrega nada al usuario final. Ver el ADR "El rol Cliente se mantiene" en el artifact del roadmap.
**Lo que sigue vivo de este ADR:** la parte de multi-rol (`user_roles`) es real y necesaria para el **staff interno** (`Nuevo #3`) — un usuario del equipo hoy solo puede tener un `role` a la vez. Esa migración se hace junto con ADR-011 (CRUD de roles + permisos granulares), no por separado — es la misma tabla puente y el mismo reemplazo de `lib/authz.ts`.
**Relacionado:** [Rec. #24], [Rmap #22] (cerrados 2026-09-16, el rol CLIENTE se mantiene) — ver ADR-011 para el trabajo activo.

### ADR-006 — Un solo dominio público (probablemente ya resuelto por ADR-004)
**Estado:** CONFIRMADA (cerrada 2026-09-16, `Nuevo #1`)
**Motivo original:** evitar SSO cruzado entre dos dominios si Codia Task seguía viviendo como servicio aparte.
**Por qué ya no aplica en la práctica:** ADR-004 (2026-09-01) confirmó que el backend Express de Codia Task se apaga una vez portado lo útil a RGC — no queda un segundo servicio con el que compartir dominio. Todo vive en un único proyecto Next.js (RGC), un solo dominio, una sola cookie de sesión — no hay nada que coordinar entre dos dominios. Confirmado explícitamente por el dueño del proyecto el 2026-09-16.
...
**Reconfirmado (2026-09-16):** el alta de usuarios sigue siendo manual desde `/admin/usuarios` (Admin crea la cuenta, la persona define su contraseña). Mientras ese sea el mecanismo, no hay ninguna dependencia de Google Cloud que resolver — ADR-008 y ADR-009 quedan en espera, no bloqueadas por nada externo, simplemente sin trabajo que hacer todavía porque no existe un camino de login que auto-cree o auto-autorice acceso.

### ADR-008 — El SSO interno debe soportar múltiples dominios corporativos, no uno solo
**Estado:** CONFIRMADA como requisito (2026-08-28) — **en espera, sin trabajo pendiente mientras el alta de usuarios sea manual (reconfirmado 2026-09-16, ver ADR-007)**
...
**Estado:** PROPUESTA — **en espera, sin urgencia mientras el alta sea manual (reconfirmado 2026-09-16, ver ADR-007)**
